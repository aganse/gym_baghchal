...
int outputString(char player, int skill, int round, int boardarray[25],
  ...
  char boardarraystr[26]="abcdefghijklmnopqrstuvwxy";
  ...
  /*
  example output:
  player=T\&skill=1\&round=002\&boardarray=1000100000100002000000001\&goatstaken=00\&move=a1%2Cb1
  */

  /*memcpy(boardarraystr, boardarray, 25);
  boardarraystr[25] = '\0';*/
  for (i=0; i<25; i++)
         index += sprintf(&boardarraystr[index], "%d", boardarray[i]);

  printf("player=%c\\&skill=%d\\&round=%03d\\&boardarray=%25s\\&goatstaken=%02d\n",
         player,skill,round,boardarraystr,goatstaken);
